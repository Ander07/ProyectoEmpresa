...
function searchProductB(req,res){
...
function removeProduct(req, res){
    ...
    Branch.findOneAndUpdate(branchId, {$pull:{products:productId}}, {new:true}, 
    (err, productRemoved)=>{
        if(err){
            res.status(500).send({message: 'Error general'});
        }else if(productRemoved){
            res.send({branch: productRemoved});
        }else{
            res.status(418).send({message: 'Producto no eliminado no eliminado'});
        }
    });
